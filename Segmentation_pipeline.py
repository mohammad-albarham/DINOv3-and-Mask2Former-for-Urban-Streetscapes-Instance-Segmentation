...
def get_patch_labels(label_np, patch_size=16, num_classes=num_classes):
    ...
    mask_tensor = torch.from_numpy(label_np)[None, None].float() # torch.Size([1, 1, 224, 224])
    patch_labels_tensor = F.interpolate(
        mask_tensor, size=(grid_h, grid_w), mode="nearest"
    )[0, 0].long() # For each patch (block of 16×16 pixels in the original), you look at the first pixel (top-left) of that patch, take its label, and that becomes the label of the whole patch.
    patch_labels = patch_labels_tensor.view(-1).numpy()
    patch_labels[patch_labels >= num_classes] = 255  # ignore index for out of bounds
    return patch_labels
...
def extract_patch_features(image,num_register_tokens=0):
    ...
    patch_features = outputs.last_hidden_state[:, 1 + num_register_tokens:, :].squeeze(0).cpu().numpy()
...
epochs = 20  # Or use wandb_config["epochs"]
...
for epoch in range(epochs):
    ...
    for img_path, lbl_path in zip(image_files, label_files):
        ...
        rprint_debug("patch_features_tensor.shape:", patch_features_tensor.shape)
        rprint_debug("patch_labels_tensor.shape:", patch_labels_tensor.shape)
...
for idx, (test_img_path, test_lbl_path) in enumerate(zip(image_files, label_files)):
    ...
    # Fix: convert figure to numpy RGB image via renderer.buffer_rgba()
    fig.canvas.draw()
