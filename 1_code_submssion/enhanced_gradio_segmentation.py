...
def gradio_inference(image, threshold, mask_transparency):
    ...
    try:
        ...
        for cid, cname in CLASS_NAMES.items():
            ...
            legend_html += f"""
            <div style='display:flex;align-items:center;padding:4px 8px;background:white;border-radius:4px;border:1px solid #ddd;margin-bottom:3px;'>
                <div style='width:22px;height:22px;background:rgb{tuple(color)};border-radius:3px;margin-right:10px;border:1.5px solid #333;'></div>
                <span style='font-size:16px;font-weight:700;color:#202020;text-shadow:0 1px 1px #fff,0 -1px 4px #0003;'>{cname}</span>
            </div>
            """
        legend_html += "</div>"
